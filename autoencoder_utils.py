...
def print_epoch(epoch, loss, num_items, dataset_size, elapsed_time, epoch_type="Training", end='\r'):
...
def _train_epoch(model, optimizer, loss_fn, data_loader, epoch, log_interval=1):
    ...
    for batch_idx, (data, _) in enumerate(data_loader):
        ...
        if batch_idx % log_interval == 0:
            print_epoch(epoch, epoch_loss, num_items, dataset_size, time.time()-start)

    print_epoch(epoch, epoch_loss, num_items, dataset_size, time.time()-start, end='\n')
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, _) in enumerate(data_loader):
            ...
            if batch_idx % log_interval == 0:
                ...
                print_epoch(epoch, epoch_loss, num_items, dataset_size, elapsed_time, epoch_type="Verification")

        elapsed_time = time.time() - start
        print_epoch(epoch, epoch_loss, num_items, dataset_size, elapsed_time, epoch_type="Verification", end='\n')
...
def train_model(model, optimizer, loss_fn, n_epochs, train_loader, test_loader, verify=False, trainingpath='./models', name='model'):
    ...
    for epoch in range(n_epochs):
        ...
        model.run_epoch(optimizer, loss_fn, train_loader, epoch, training=True)
        if verify:
            model.run_epoch(optimizer, loss_fn, test_loader, epoch, training=False)
